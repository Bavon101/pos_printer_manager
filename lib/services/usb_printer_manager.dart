...
class USBPrinterManager extends PrinterManager {
  ...
  writeBytes(List<int> data, {bool isDisconnect = true}) async {
    if (Platform.isWindows) {
      ...
      await Future.forEach<List<int>>(datas,
          (data) async => await usbPrinter.write(Uint8List.fromList(data)));
...
extension on List<int> {
  ...
  List<List<int>> chunkBy(num value) {
    ...
    if (size <= value) {
      ...
      for (var i = 0; i < max; i++) {
        int startIndex = (value * i) as int;
        int endIndex = (value * (i + 1)) as int;
        if (endIndex > size) {
          endIndex = size;
        }
